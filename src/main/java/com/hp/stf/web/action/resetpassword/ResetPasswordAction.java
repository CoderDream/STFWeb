package com.hp.stf.web.action.resetpassword;
...
public class ResetPasswordAction extends BaseAction {
	...
	public String doReset() {
		...
		String password = resetPasswordVo.getOldPassword();

		User user = userService.findUser(user_name, password);

		if (null != user) {
			System.out.println(resetPasswordVo.getNewPassword());

			int i = userService.updateUserPassword(user_name,
			...
			if (1 == i) {
				...
				addActionMessage("Password Reset Failure!");
