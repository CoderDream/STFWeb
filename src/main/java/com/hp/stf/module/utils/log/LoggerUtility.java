package com.hp.stf.module.utils.log;
...
public class LoggerUtility {
	...
	public void endInvoke(String methodName) {
		...
		logger.fatal(String.format("[End   Local API Invoked][-][lvl:%-2d]-----%s(), elapsed time: %d"
					, invokeInfo.getLevel(), methodName, new Date().getTime() - invokeInfo.getDate().getTime() ) );	
	}
	...
	public void endPerformanceLog() {
		...
		String info = String.format("[Performance Log]---->[%s]---->End, elapsed time: %d"
						, invokeInfo.getInvokerName(), new Date().getTime() - invokeInfo.getDate().getTime() ); 
		logger.fatal(info);
	}
}
